...
// Highlighter :
		...
		highlightEnabled(true)
	{
		...
		// GLIP LayoutLoader Keywords : 	
			...
			{
				rule.pattern = QRegExp(tr("\\b%1\\b").arg(Glip::Modules::LayoutLoader::getKeyword(static_cast<Glip::Modules::LayoutLoaderKeyword>(i))));
		...
		// GLIP Uniform Loader Keywords : 
			...
			{
				rule.pattern = QRegExp(tr("\\b%1\\b").arg(Glip::Modules::UniformsLoader::getKeyword(static_cast<Glip::Modules::UniformsLoaderKeyword>(i))));
...
// ElementsMenu :
	...
	void ElementsMenu::update(const Glip::Modules::LayoutLoader::PipelineScriptElements& elements, const QFileInfo& fileInfo)
	{
		QAction* ptr = NULL;
		// Rebuild the menu :
		clear();
		...
		if(!fileInfo.fileName().isEmpty())
		{
			...
			signalMapper.setMapping(ptr, tr("%1(%2)\n").arg(Glip::Modules::LayoutLoader::getKeyword(Glip::Modules::KW_LL_INCLUDE)).arg(fileInfo.fileName()));
		}
		else
			setTitle("<Unnamed>");
		if(!fileInfo.path().isEmpty())
		{
			ptr = addAction(tr("Add path..."), &signalMapper, SLOT(map()));
			signalMapper.setMapping(ptr, tr("%1(%2)\n").arg(Glip::Modules::LayoutLoader::getKeyword(Glip::Modules::KW_LL_ADD_PATH)).arg(fileInfo.path()));
		}
		...
		#define MAKE_LIST( listName, name ) \
			...
			{ \
				...
				{ \
					ptr = tmp->addAction(QString::fromStdString(elements. listName [k]), &signalMapper, SLOT(map())); \
					signalMapper.setMapping(ptr, QString::fromStdString(elements. listName [k])); \
				} \
			}
		...
		if(!elements.pipelines.empty())
		{
			...
			{
				ptr = tmp->addAction(QString::fromStdString(elements.pipelines[k]), &signalMapper, SLOT(map()));
				signalMapper.setMapping(ptr, QString::fromStdString(elements.pipelines[k]));

				QMenu* i = tmp->addMenu(tr("Inputs of %1 (%2)").arg(QString::fromStdString(elements.pipelines[k])).arg(elements.pipelineInputs[k].size()));
				for(unsigned int ki=0; ki<elements.pipelineInputs[k].size(); ki++)
				{
					ptr = i->addAction(QString::fromStdString(elements.pipelineInputs[k][ki]), &signalMapper, SLOT(map()));
					signalMapper.setMapping(ptr, QString::fromStdString(elements.pipelineInputs[k][ki]));
				}

				QMenu* o = tmp->addMenu(tr("Outputs of %1 (%2)").arg(QString::fromStdString(elements.pipelines[k])).arg(elements.pipelineOutputs[k].size()));
				for(unsigned int ko=0; ko<elements.pipelineOutputs[k].size(); ko++)
				{
					ptr = o->addAction(QString::fromStdString(elements.pipelineOutputs[k][ko]), &signalMapper, SLOT(map()));
					signalMapper.setMapping(ptr, QString::fromStdString(elements.pipelineOutputs[k][ko]));
				}
			}
		}
		...
		if(!elements.mainPipeline.empty())
		{
			...
			ptr = tmp->addAction(QString::fromStdString(elements.mainPipeline), &signalMapper, SLOT(map()));
			signalMapper.setMapping(ptr, QString::fromStdString(elements.mainPipeline));

			QMenu* i = tmp->addMenu(tr("Inputs of %1 (%2)").arg(QString::fromStdString(elements.mainPipeline)).arg(elements.mainPipelineInputs.size()));
			for(unsigned int ki=0; ki<elements.mainPipelineInputs.size(); ki++)
			{
				ptr = i->addAction(QString::fromStdString(elements.mainPipelineInputs[ki]), &signalMapper, SLOT(map()));
				signalMapper.setMapping(ptr, QString::fromStdString(elements.mainPipelineInputs[ki]));
			}

			QMenu* o = tmp->addMenu(tr("Outputs of %1 (%2)").arg(QString::fromStdString(elements.mainPipeline)).arg(elements.mainPipelineOutputs.size()));
			for(unsigned int ko=0; ko<elements.mainPipelineOutputs.size(); ko++)
			{
				ptr = o->addAction(QString::fromStdString(elements.mainPipelineOutputs[ko]), &signalMapper, SLOT(map()));
				signalMapper.setMapping(ptr, QString::fromStdString(elements.mainPipelineOutputs[ko]));
			}
		}
	}
...
// CodeEditor :
	...
	{
		while(!subContextMenus.isEmpty())
			delete subContextMenus.first();
		while(!managedMenus.isEmpty())
			delete subContextMenus.first();
		delete highLighter;
	...
	void CodeEditor::lineNumberAreaPaintEvent(QPaintEvent *event)
	{
		...
		const QColor 	backgroundColor = (palette().background().color().lightnessF()>=0.5) ? palette().background().color().darker(130) : palette().background().color().lighter(130),
				color = (palette().text().color().lightnessF()>=0.5) ? palette().text().color().lighter(130) : palette().text().color().darker(130);
		...
		while(block.isValid() && top<=event->rect().bottom())
		{
			if(block.isVisible() && bottom>=event->rect().top())
			{
				...
				{	
					...
					tmpPen.setColor( (palette().background().color().lightnessF()>=0.5) ? Qt::black : Qt::white );
				...
				if(errorLineNumbers.contains(blockNumber+1))
				{
					QFont tmpFont = painterFont;
					tmpFont.setBold(true);	
					QPen tmpPen = painterPen;
	...
	void CodeEditor::updateExtraHighlight(void)
	{
		if(!isReadOnly())
		{
			const QColor lineColor = (palette().background().color().lightnessF()>=0.5) ? palette().background().color().darker(110) : palette().background().color().lighter(150);
	...
	void CodeEditor::updateElements(void)
	{
		if(!managedMenus.isEmpty())
		{
			// Read : 
			Glip::Modules::LayoutLoader layoutLoader;
			Glip::Modules::LayoutLoader::PipelineScriptElements elements;
			bool test = true;
			try
			{
				elements = layoutLoader.listElements(getContent());
			}
			catch(Glip::Exception& e)
			{
				test = false; // Forget the possible Exceptions
			}

			// Update all managed menus :
			if(test)
			{
				for(QVector<ElementsMenu*>::iterator it=managedMenus.begin(); it!=managedMenus.end(); it++)
					(*it)->update(elements, getFileInfo());
			}
		}	
	}
	
	void CodeEditor::subContextMenuDestroyed(void)
	{
		QMenu* subMenu = reinterpret_cast<QMenu*>(QObject::sender());
		const int index = subContextMenus.indexOf(subMenu);
		if(index>=0)
			subContextMenus.remove(index);
	}

	void CodeEditor::elementsMenuDestroyed(void)
	{
		ElementsMenu* managedMenu = reinterpret_cast<ElementsMenu*>(QObject::sender());
		const int index = managedMenus.indexOf(managedMenu);
		if(index>=0)
	...
	bool CodeEditor::isEmpty(void) const
	{
		return document()->isEmpty();
	}

	const QFileInfo& CodeEditor::getFileInfo(void) const
	{
		return fileInfo;
	} 
	std::string CodeEditor::getContent(void) const
	{
	...
	void CodeEditor::open(QString newFilename)
	{
		...
		const bool prevState = this->blockSignals(true);
	...
	void CodeEditor::addSubContextMenu(QMenu* menu)
	{
		if(!subContextMenus.contains(menu))
		{
			QObject::connect(menu, SIGNAL(destroyed()), this, SLOT(subContextMenuDestroyed()));
			subContextMenus.append(menu);
		}
	}

	ElementsMenu* CodeEditor::createManagedMenu(void)
	{
		ElementsMenu* elementsMenu = new ElementsMenu(this);
		QObject::connect(elementsMenu, SIGNAL(destroyed()), this, SLOT(elementsMenuDestroyed()));
		managedMenus.push_back(elementsMenu);
		return elementsMenu;
	}
	...
	void CodeEditor::highlightErrorLines(const QVector<int>& lineNumbers)
	{
...
/*	CodeEditorContainer::CodeEditorContainer(QWidget* parent)
	...
	void CodeEditorContainer::showErrors(Glip::Exception compilationError)
	{
		...
		{
			const Glip::Exception& e= (k==compilationError.getNumSubExceptions()) ? compilationError : compilationError.getSubException(k);
			...
			if(e.getType()==Glip::Exception::ClientShaderException || e.getType()==Glip::Exception::ClientScriptException)
			{
			...
			if(e.getType()==Glip::Exception::ClientShaderException || e.getType()==Glip::Exception::ClientScriptException || k==0)
			{
...
// CodeEditorSettings :	
	...
	void CodeEditorSettings::loadSettings(void)
	{
		...
		READ_SETTING(glslPreprocessorColor, value<QColor>() )
		READ_SETTING(glipLayoutLoaderKeywordColor, value<QColor>() )
		READ_SETTING(glipUniformLoaderKeywordColor, value<QColor>() )
		READ_SETTING(commentsColor, value<QColor>() )
		READ_SETTING(braceMatchingColor, value<QColor>() )
...
// CodeEditorSettingsWidget :
	CodeEditorSettingsWidget::CodeEditorSettingsWidget(void)
	 : 	settings( fontInfo().pointSize() ),
		...
		layoutFonts(NULL),
		layoutMisc(NULL),
		glslKeywordColorLabel("GLSL Keywords", this),
		glslFunctionColorLabel("GLSL Functions", this),
		glslPreprocessorColorLabel("GLSL Preprocessor", this),
		glipLayoutLoaderKeywordColorLabel("GLIP Layout Loader Keywords", this),
		glipUniformLoaderKeywordColorLabel("GLIP Uniforms Loader Keywords", this),
		commentsColorLabel("Comments", this),
		braceMatchingColorLabel("Matching braces", this),
		searchColorLabel("Search", this),
		glslKeywordColorButton(this),
		glslFunctionColorButton(this),
		glslPreprocessorColorButton(this),
		glipLayoutLoaderKeywordColorButton(this),
		glipUniformLoaderKeywordColorButton(this),
		commentsColorButton(this),
		searchColorButton(this),
		braceMatchingColorButton(this),
		editorFontButton(this),
		keywordFontButton(this),
		dialogButtons(QDialogButtonBox::Ok | QDialogButtonBox::Apply | QDialogButtonBox::Reset | QDialogButtonBox::Cancel, Qt::Horizontal, this),
	...
	void CodeEditorSettingsWidget::updateInterface(void)
	{
		// From Values to GUI...
	...
	void CodeEditorSettingsWidget::changeColor(void)
	{
		// Get the pushbutton : 
		...
		if(target==&glslKeywordColorButton)
			...
			throw Glip::Exception("CodeEditorSettingsWidget::changeColor - Unknown color picker (internal error).", __FILE__, __LINE__);
	...
	void CodeEditorSettingsWidget::changeFont(void)
	{
		QPushButton* target = reinterpret_cast<QPushButton*>(QObject::sender());
		...
		if(target==&editorFontButton)
			...
			throw Glip::Exception("CodeEditorSettingsWidget::changeFont - Unknown color picker (internal error).", __FILE__, __LINE__);
	...
	void CodeEditorSettingsWidget::processDialogButtonPressed(QAbstractButton* button)
	{
	...
	const CodeEditorSettings& CodeEditorSettingsWidget::getSettings(void) const
	{
		return settings;
	}

	void CodeEditorSettingsWidget::resetSettings(void)
	{
...
// CodeEditorTabs :
	...
	CodeEditorSettingsWidget& CodeEditorTabs::getEditorSettingsInterface(void)
	{
	...
	void CodeEditorTabs::compilationFailureNotification(void* identifier, Glip::Exception compilationError)
	{
		CodeEditorContainer* editor = reinterpret_cast<CodeEditorContainer*>(identifier);
