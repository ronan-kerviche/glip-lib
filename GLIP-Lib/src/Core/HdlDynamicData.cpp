	...
	GLenum HdlDynamicData::getRelatedGLSupportingType(const GLenum& t)
	{
		switch(t)
		{
			...
			case GL_FLOAT_MAT4 :
							return GL_FLOAT;
			default :
				throw Exception("HdlDynamicData::getRelatedGLSupportingType - Unknown GL type identifier : \"" + glParamName(t) + "\" (internal error).", __FILE__, __LINE__, Exception::CoreException);
		}
	}
	...
	**/
	HdlDynamicData* HdlDynamicData::build(const GLenum& type)
	{
		...
		#define GENERATE_ELM( glType, CType, _rows, _cols) \
			...
			throw Exception("HdlDynamicData::build - Unknown GL type identifier : \"" + glParamName(type) + "\".", __FILE__, __LINE__, Exception::CoreException);
	...
	**/
	HdlDynamicData* HdlDynamicData::copy(const HdlDynamicData& cpy)
	{
		...
		#define COPY_ELM( glType, CType, _rows, _cols) \
			...
			throw Exception("HdlDynamicData::copy - Unknown GL type identifier : \"" + glParamName(cpy.getGLType()) + "\" (internal error).", __FILE__, __LINE__, Exception::CoreException);
	...
	**/
	HdlDynamicTable* HdlDynamicTable::build(const GLenum& type, const int& _columns, const int& _rows, const int& _slices, bool _normalized, int _alignment)
	{
		...
		#define ERROR_ELM(glType) \
			if(type== glType ) \
				throw Exception("HdlDynamicTable::build - Unable to build a table of type \"" + glParamName(type) + "\" : Illegal type.", __FILE__, __LINE__, Exception::CoreException);
			...
			throw Exception("HdlDynamicTable::build - Unknown GL type identifier : \"" + glParamName(type) + "\".", __FILE__, __LINE__, Exception::CoreException);
	...
	**/
	HdlDynamicTable* HdlDynamicTable::copy(const HdlDynamicTable& cpy)
	{
		...
		#define COPY_ELM( glType, CType) \
			...
			throw Exception("HdlDynamicTable::copy - Unknown GL type identifier : \"" + glParamName(cpy.getGLType()) + "\" (internal error).", __FILE__, __LINE__, Exception::CoreException);
